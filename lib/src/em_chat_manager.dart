...
//        message.msgId = response['ServerMsgId'];
  ...
  void _addNativeMethodCallHandler() {
    _emChatManagerChannel.setMethodCallHandler((MethodCall call) {
      ...
      //------------------------------------------------------------------------
      //------------------------------------------------------------------------
      else if(call.method == "onCallState"){
        return _onCallState(argMap);
      } else if(call.method == "onCallReceive"){
        return _onCallReceive(argMap);
      }
      return null;
    });
  }
  ...
  final _emCallManagerListeners = List<EMCallManagerListener>();
  final _emCallStateListeners = List<EMCallStateListener>();

  /// 监听电话接入
  Future<void> _onCallReceive(Map map) async {
    var from = map["from"];
    for (var listener in _emCallManagerListeners) {
      listener.onCallReceives(from);
    }
  }

  /// 接收通话状态
  Future<void> _onCallState(Map map) async {
    var status = map['status'];
    for (var listener in _emCallStateListeners) {
      listener.onCallState(status);
    }
  }

  /// 拨打电话.
  void makeVoiceCall(String username){
    _emChatManagerChannel.invokeMethod(
        "makeVoiceCall",{"username" : username});
//    result.then((response){
//      if (response["success"]) {
////        message.msgId = response['ServerMsgId'];
//        message.msgId = EMMessage.from(response['message']).msgId;
...
//    });
  ...
  Future<bool> getInComingCall() async {
    bool result = await _emChatManagerChannel.invokeMethod(
      "getInComingCall");
    return result;
  }

  /// 注册电话监听.
  void addCallReceiverListener(EMCallManagerListener listener){
//    _emChatManagerChannel.invokeMethod(
//        "addCallReceiverListener");
    _emCallManagerListeners.add(listener);
  }

  /// 移除所有电话监听.
  void removeCallReceiverListener(EMCallManagerListener listener){
//    _emChatManagerChannel.invokeMethod(
//        "removeCallReceiverListener");
    _emCallManagerListeners.remove(listener);
  }

  /// 注册通话状态监听.
  void addCallStateChangeListener(EMCallStateListener listener){
//    _emChatManagerChannel.invokeMethod(
//        "addCallStateChangeListener");
    _emCallStateListeners.add(listener);
  }

  /// 移除通话状态监听.
  void removeCallStateChangeListener(EMCallStateListener listener){
//    _emChatManagerChannel.invokeMethod(
//        "removeCallStateChangeListener");
    _emCallStateListeners.remove(listener);
  }
